...
function getOsLinksFromRelease(release) {
  ...
  const linux = release?.assets?.filter(asset => asset?.['name'] === "runik_linux.flatpak")?.[0]?.['browser_download_url']
  const windows = release?.assets?.filter(asset => asset?.['name'] === "runik_windows.exe")?.[0]?.['browser_download_url']
